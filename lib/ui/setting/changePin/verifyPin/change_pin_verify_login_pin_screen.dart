...
class ChangePinVerifyLoginPinScreen extends ConsumerStatefulWidget {
  const ChangePinVerifyLoginPinScreen({
    Key? key,
  }) : super(key: key);

  @override
  ConsumerState<ChangePinVerifyLoginPinScreen> createState() =>
      _ChangePinVerifyLoginPinScreenState();
}

class _ChangePinVerifyLoginPinScreenState
    extends ConsumerState<ChangePinVerifyLoginPinScreen> {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: BaseWidget<LoginPinScreenVm>(
          model: LoginPinScreenVm(ref.read(authRepositoryProvider),
              ref.read(tokenBoxRepositoryProvider), ref.watch(userBoxProvider),ref.watch(linkBoxProvider)),
          onModelReady: (model) {
            loginPinModel = model;
          },
  ...
  void _onSuccess(String message) {
    Navigator.pushNamed(context, RouteName.changePinScreen, arguments: {
      "logInPin": enteredMPin,
    });
  }
